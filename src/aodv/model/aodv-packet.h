...
public:
  ...
  uint32_t GetEnergy(void) const {return m_energy;}
  float GetValue(void) const {return m_value;}
  uint32_t GetQueuelength (void) const {return m_queuelength;} 
  uint32_t GetMnsEnergy (void) const {return mns_energy;}
  void SetMnsEnergy (uint32_t e) {mns_energy = e;}
  void SetEnergy (uint32_t e);
  void SetValue (float e) {m_value = e;}
  void SetQueueLength (uint32_t e) {m_queuelength = e;}
  uint32_t GetValue_S(void) const {uint32_t re; memcpy(&re, &m_value, sizeof(float)); return re;}
  void SetValue_DS(uint32_t e) {float re; uint32_t rre = e; memcpy(&re, &rre, sizeof(float));SetValue(re);}
...
private:
  ...
  uint32_t m_energy; // range from 0-1000000 (1 million)
  float m_value;
...
class addHeader : public Header {
public:
    // constructor:
    // addHeader();
    addHeader(Ipv4Address previousHop = Ipv4Address(),uint32_t previousEnergy = 0,Ipv4Address dstAddress = Ipv4Address());
    static TypeId GetTypeId ();
    TypeId GetInstanceTypeId () const;
    uint32_t GetSerializedSize () const;
    void Serialize (Buffer::Iterator start) const;
    ...
    //Mods:
    void SetPreviousHop (Ipv4Address pre) {previousHop = pre;}
    Ipv4Address GetPreviousHop (void) const {return previousHop;}
    void SetPreviousEnergy (uint32_t e) {previousEnergy = e;}
    uint32_t GetPreviousEnergy (void) const {return previousEnergy;}
    void SetDstAddress (Ipv4Address dst) {dstAddress = dst;}
    Ipv4Address GetDstAddress (void) const {return dstAddress;}
