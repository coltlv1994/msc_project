...
RrepHeader::Serialize (Buffer::Iterator i) const
{
  ...
  i.WriteHtonU32 (m_energy);
  i.WriteHtonU32 (GetValue_S());
  i.WriteHtonU32 (m_queuelength);
  i.WriteHtonU32 (mns_energy);
}
...
RrepHeader::Deserialize (Buffer::Iterator start)
{
  ...
  m_energy = i.ReadNtohU32 ();
  SetValue_DS(i.ReadNtohU32 ());
  m_queuelength = i.ReadNtohU32 ();
  mns_energy = i.ReadNtohU32 ();
...
void RrepHeader::SetEnergy (uint32_t e) {
  m_energy = e;
}
...
// new aodv-header:
addHeader::addHeader (Ipv4Address pre, uint32_t en, Ipv4Address dst) {
  previousHop = pre;
  previousEnergy = en;
  dstAddress = dst;
}

NS_OBJECT_ENSURE_REGISTERED (addHeader);

TypeId addHeader::GetTypeId(void) {
  static TypeId tid = TypeId ("ns3::addHeader").SetParent<Header> ().SetGroupName("Aodv").AddConstructor<addHeader> ();
  return tid;
}

TypeId addHeader::GetInstanceTypeId(void) const {
  return GetTypeId();
}

uint32_t addHeader::GetSerializedSize(void) const {
  // Ipv4Address : 32 bit
  // uint32_t : 32 bit
  return 12;
}

void addHeader::Serialize(Buffer::Iterator i) const {
  WriteTo(i,previousHop);
  i.WriteHtonU32(previousEnergy);
  WriteTo(i,dstAddress);
}

uint32_t addHeader::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
  // i.Next(8);

  ReadFrom(i, previousHop);
  previousEnergy = i.ReadNtohU32();
  ReadFrom(i, dstAddress);
  
  uint32_t ddsstt = i.GetDistanceFrom(start);
  NS_ASSERT (ddsstt == GetSerializedSize ());
  return ddsstt;
}

void addHeader::Print(std::ostream & os) const {
  os << "Previous Hop:" << previousHop << "\nWith energy:" << previousEnergy << "\nDestination:" << dstAddress << std::endl;
}
