...
{
  //this function correspones with the sending of packet.
  NS_LOG_FUNCTION (this << header << (oif ? oif->GetIfIndex () : 0));
//  addHeader ah;
//  p->PeekHeader(ah);
//  NS_LOG_UNCOND(ah.GetDstAddress());
  if (!p)
    {
  ...
  // Ptr<Node> nd = m_lo->GetNode();
  // Ptr<NetDevice> dev= nd->GetDevice(0);
  // route->SetOutputDevice(dev);
  // route->SetDestination(dst);
  // route->SetSource(src);
  // route->SetGateway(m_nb.RouteDecision(nd,ah.GetDstAddress(),dst));
  // return route;
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  // Forwarding
  // NS_LOG_UNCOND(idev->GetNode()->GetSelfEnergy());
...
RoutingProtocol::Forwarding (Ptr<const Packet> p, const Ipv4Header & header,
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  // Time pa = Simulator::Now();
  NS_LOG_FUNCTION (this);
  m_lo->GetNode()->SelfEnergyInDe(100,false);

  // NS_LOG_UNCOND(m_queue.GetSize());
  Ipv4Address dst = header.GetDestination ();
  Ipv4Address origin = header.GetSource ();

  Ptr<Ipv4Route> m_route = Create<Ipv4Route>();

  m_route->SetSource(origin);
  m_route->SetDestination(dst);

  Ptr<Node> nd = m_lo->GetNode();
  Ptr<NetDevice> dev = nd->GetDevice(0);
  m_route->SetOutputDevice(dev);

  // The original packet is a const object,
  // make a copy of it, and send the replica.
  Ptr<Packet> packet;
  packet = p->Copy();
  addHeader ah;
  packet->RemoveHeader(ah);
  Ipv4Address pr = ah.GetDstAddress();
  m_route->SetGateway(m_nb.RouteDecision(nd,pr,dst)); 
  // NS_LOG_UNCOND("NextHop: "<<m_route->GetGateway());
  // NS_LOG_UNCOND("Previous Hop: "<<ah.GetDstAddress());
  ah.SetDstAddress(nd->GetObject<Ipv4>()->GetAddress(1,0).GetLocal());
  NS_LOG_UNCOND("S:"<<nd->GetObject<Ipv4>()->GetAddress(1,0).GetLocal()<<"\tEnergy:"<<nd->GetSelfEnergy());
  NS_LOG_UNCOND(m_nb.PrintOut());
  // NS_LOG_UNCOND(m_maxQueueTime);
  // NS_LOG_UNCOND(m_maxQueueLen);
  // NS_LOG_UNCOND(m_pathDiscoveryTime);
  packet->AddHeader(ah);

  m_nb.Update (m_route->GetGateway (), m_activeRouteTimeout);
  m_nb.Update (pr, m_activeRouteTimeout);
  ...
  */
  ...
  ucb (m_route, packet, header);
            // ucb (toOri_route, p, header);
            return true;
  // NS_LOG_UNCOND(Simulator::Now()-pa);
}

void RoutingProtocol::QLearningDeciding(void) {
  return;
}
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  // receiver.Print(a);
  // NS_LOG_UNCOND(a.str());
  // NS_LOG_UNCOND will show the output no matter what happened, recom-.
  RreqHeader rreqHeader;
   ...
   */
  RoutingTableEntry toOrigin;
  // LookupRoute will return bool value: true for success, false for failure.
  if (!m_routingTable.LookupRoute (origin, toOrigin))
    {
      // if find no entry to origin, add the new entry;
      Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
    ...
    {
      // if find entry to origin, update the entry
...
RoutingProtocol::ProcessHello (RrepHeader const & rrepHeader, Ipv4Address receiver )
{
  ...
  m_lo->GetNode()->SelfEnergyInDe(50,false);
  /*
  ...
  if (m_enableHello)
    {
      // NS_LOG_UNCOND((float)rrepHeader.GetValue()/10000000.0);
      m_nb.Update (rrepHeader.GetDst (), Time (m_allowedHelloLoss * m_helloInterval),rrepHeader.GetEnergy(), rrepHeader.GetValue(), rrepHeader.GetQueuelength(),rrepHeader.GetMnsEnergy());
      // NS_LOG_UNCOND(std::max (Time (m_allowedHelloLoss * m_helloInterval), toNeighbor.GetLifeTime ()));
    }
}
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      // NS_LOG_UNCOND(m_lo->GetNode()->GetSelfEnergy());
      helloHeader.SetEnergy(m_lo->GetNode()->GetSelfEnergy());
      helloHeader.SetValue(m_lo->GetNode()->GetSelfV_value());
      helloHeader.SetQueueLength(0);
      // NS_LOG_UNCOND(m_lo->GetNode()->GetSelfEnergy());
      // NS_LOG_UNCOND(m_nb.PrintOut());
      // NS_LOG_UNCOND(std::endl);
      helloHeader.SetMnsEnergy(m_nb.GetMnsEnergy());
      Ptr<Packet> packet = Create<Packet> ();
      SocketIpTtlTag tag;
      tag.SetTtl (1);
      packet->AddPacketTag (tag);
