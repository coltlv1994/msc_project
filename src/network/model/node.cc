...
Node::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::Node")
    ...
    .AddAttribute ("SystemId", "The systemId of this node: a unique integer used for parallel simulations.",
                   ...
                   MakeUintegerChecker<uint32_t> ())
                   // new lines, Sept. 29, 2017
    .AddAttribute("NodeEnergy", "The energy level of this node: a non-negative floating-point value.",
                  TypeId::ATTR_GET || TypeId::ATTR_SET,
                  UintegerValue (0),
                  MakeUintegerAccessor (&Node::nodeEnergy),
                  MakeUintegerChecker<uint32_t> ())
    .AddAttribute("NodeValue", "The value of this node: a non-negative floating-point value.",
                  TypeId::ATTR_GET || TypeId::ATTR_SET,
                  IntegerValue(0),
                  MakeIntegerAccessor (&Node::nodeEnergy),
...
//changelog: Sept. 28, 2017. Zhengyang Lyu
// Added functions;

uint32_t Node::GetSelfEnergy (void) {
  // NS_LOG_FUNCTION (this);
  return nodeEnergy;
}

float Node::GetSelfV_value (void) {
  // NS_LOG_FUNCTION (this);
  return nodeV_value;
}

void Node::SetSelfEnergy(uint32_t e) {
  NS_LOG_FUNCTION(this);
  nodeEnergy = e;
}

void Node::SetSelfV_value(float e) {
  NS_LOG_FUNCTION(this);
  nodeV_value = e;
}

void Node::SelfEnergyInDe(uint32_t e, bool i_d) {
  // boolean value here indicates the increasement or decreasement:
  // true for adding, false for losing.
  if(i_d==true) {
    nodeEnergy += e;
  }
  else {
