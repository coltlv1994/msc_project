...
// uint32_t Packet::RemoveHeader(aodv::addHeader &ah) {
//   uint32_t deserialized = ah.Deserialize (m_buffer.Begin ());
//   NS_LOG_UNCOND("NEW FUNCTION!");
//   NS_LOG_FUNCTION (this << ah.GetInstanceTypeId ().GetName () << deserialized);
//   m_buffer.RemoveAtStart (deserialized);
//   m_byteTagList.Adjust (-deserialized);
//   m_metadata.RemoveHeader (ah, deserialized);
//   return deserialized;
// }

uint32_t
Packet::RemoveHeader (Header &header, bool adh)
{
  if(adh==false) {
    return Packet::RemoveHeader(header);
  }
  else {
    uint32_t deserialized = header.Deserialize (m_buffer.Begin ());
    NS_LOG_FUNCTION (this << header.GetInstanceTypeId ().GetName () << deserialized);
