...
public:
  ...
  functions that allows the node to complete the works
  */
  uint32_t GetSelfEnergy (void) ;

  float GetSelfV_value (void) ;

  void SetSelfEnergy (uint32_t);

  void SetSelfV_value (float);

  void SelfEnergyInDe (uint32_t, bool);

  // End of change, Sept. 28, 2017
...
private:
  ...
  uint32_t    m_sid;        //!< System id for this node
      // Change log: Zhengyang Lyu, lyuz@kth.se. September 29, 2017
  // Necessary storage spaces for nodes;
  uint32_t nodeEnergy;
  float nodeV_value;
  // end of change, Sept. 29, 2017
